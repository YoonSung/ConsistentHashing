...
class VirtualNodeImpl(nodeName: String, virtualNodeIndex: Int) extends Node(nodeName){
  override def key(): String = s"${nodeName}__$virtualNodeIndex"
...
class ImprovedHashRingRouter(virtualCountPerNode: Int) extends ConsistentHashRouter {
  ...
  override def addNode(node: Node): Unit = {
    val virtualNodeList = (1 to virtualCountPerNode).map { idx =>
      new VirtualNodeImpl(nodeName = node.name, virtualNodeIndex = idx)
    }
    virtualNodeList.foreach(virtualNode => ring.put(hash(virtualNode.key()), virtualNode))

    // 삭제를 위해 별도 메모리에 저장
    virtualNodesByNodeName.put(node.name, virtualNodeList)
  }

  override def deleteNode(node: Node): Unit = {
    val keys = virtualNodesByNodeName(node.name).map(node => hash(node.key()))
